export const TRUMPQUOTES = [
  "Sorry losers and haters, but my I.Q. is one of the highest.",
  "We will have so much winning, you may get bored with winning.",
  "I'm a very stable genius.",
  "China has taken advantage of us for too long. No more!",
  "Make America Great Again!",
  "The beauty of me is that I'm very rich.",
  "THIS IS A GREAT TIME TO BUY!!!",
  "THE GOLDEN RULE OF NEGOTIATING AND SUCCESS: HE WHO HAS THE GOLD MAKES THE RULES.",
  "Everything I touch turns into gold.",
  "I will build a great wall — and nobody builds walls better than me, believe me."
]

export const XIQUOTES = [
  "Hard work makes dreams come true.",
  "Snuffing others' candles won't brighten yours.",
  "Green hills are golden mountains.",
  "Innovation drives progress.",
  "We respect others' success, but we won't be bullied.",
  "China doesn't cause trouble, but we're not afraid of it.",
  "The East is rising, and the West is declining.",
  "No force can stop China's rise.",
  "Prepare for storms, even in calm seas.",
  "Visions only work when you act — not when you tweet."
];
