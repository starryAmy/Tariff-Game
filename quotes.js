export const TRUMPQUOTES = [
  "Sorry losers and haters, but my I.Q. is one of the highest—and you all know it!",
  "We will have so much winning, you may get bored with winning.",
  "I'm a very stable genius.",
  "China has taken advantage of us for too long. No more!",
  "Make America Great Again!",
  "The beauty of me is that I'm very rich.",
  "I like thinking big. If you're going to be thinking anything, you might as well think big.",
  "I try to learn from the past, but I plan for the future by focusing exclusively on the present.",
  "Sometimes by losing a battle you find a new way to win the war.",
  "Without passion, you don't have energy; without energy, you have nothing."
]

export const XIQUOTES = [
  "Happiness does not fall out of the blue and dreams will not come true by themselves. We need to be down-to-earth and work hard.",
  "Blowing out others' candles will not make yours brighter; blocking others' paths will not make you go further.",
  "Clear waters and green mountains are as valuable as mountains of gold and silver.",
  "Innovation is the primary driving force of development.",
  "We Chinese know only too well what it takes to achieve prosperity, so we applaud the achievements of others and wish them a better future.",
  "The Chinese people have always celebrated and striven to pursue the vision of peace, amity, and harmony.",
  "Corruption could lead to the collapse of the Party and the downfall of the State.",
  "China will always remain the builder of world peace, a contributor to global development, and upholder of international order.",
  "We must strengthen our sense of hardship, adhere to bottom-line thinking, and be prepared for danger in times of peace.",
  "Visions will come true only when we act."
]
